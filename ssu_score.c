...
char threadFiles[ARGNUM][FILELEN]; // -t XX[n]
...
void ssu_score(int argc, char *argv[])
{
	...
	if(mOption) {
		do_mOption(); // 문제 할당 점수 수정
		set_scoreTable(ansDir); // score_table 데이터 갱신
	}

	printf("grading student's test papers..\n"); 
	score_students(); // 학생 답안 채점
	if(iOption) // -i 옵션이 존재할 경우
		do_iOption(IDS); // IDS: 학번 문자열 배열(char **)
	return;
}
